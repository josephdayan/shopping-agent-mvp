...
export default async function AdminPage() {
  ...
  return (
    <main className="min-h-screen bg-atlas-lavender/55 px-4 py-5">
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-atlas-line bg-atlas-night px-4 py-4 text-atlas-lavender shadow-brand">
          <div className="space-y-3">
            <Link href="/" className="inline-flex items-center gap-2 text-sm text-atlas-lavender/65 hover:text-atlas-violet">
              <ArrowLeft size={16} />
              Voltar ao chat
            </Link>
            <AtlasBrand variant="dark" showDescriptor />
          </div>
          <AdminActions />
        </header>
        ...
        <section className="grid gap-5 lg:grid-cols-[1.3fr_0.7fr]">
          <div className="rounded-md border border-atlas-line bg-white shadow-soft">
            <div className="border-b border-atlas-line px-4 py-3">
              <h2 className="font-semibold">Pedidos</h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px] text-left text-sm">
                <thead className="bg-atlas-lavender font-mono text-[10px] uppercase tracking-[0.12em] text-atlas-muted">
                  <tr>
                ...
                <tbody>
                  {data.orders.map((order) => (
                    <tr key={order.id} className="border-t border-atlas-line">
                      <td className="px-4 py-3">
                        <p className="font-medium">{order.product.title}</p>
                        <p className="text-xs text-atlas-muted">{order.product.store} · {sourceLabel(order.product.source)}</p>
                      </td>
                      <td className="px-4 py-3">{order.user.name ?? order.user.phone}</td>
                      <td className="px-4 py-3">{order.paymentStatus}</td>
                      <td className="px-4 py-3">
                        <p>{order.fulfillmentStatus}</p>
                        <p className="text-xs text-atlas-muted">{fulfillmentLabel(order.fulfillmentMode)}</p>
                      </td>
                  ...
                  {!data.orders.length && (
                    <tr>
                      <td className="px-4 py-6 text-atlas-muted" colSpan={6}>Nenhum pedido ainda.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="space-y-5">
            <Panel title="Usuarios">
              <div className="space-y-3">
                {data.users.map((user) => (
                  <div key={user.id} className="rounded-md border border-atlas-line p-3 text-sm">
                    <p className="font-medium">{user.name ?? user.phone}</p>
                    <p className="text-atlas-body">{user.defaultAddress ?? "Sem endereco"}</p>
                    {user.preferences.map((preference) => (
                      <p key={preference.id} className="mt-2 text-xs text-atlas-violet">
            ...
            <Panel title="Catalogo Atlas">
              <div className="max-h-[420px] space-y-2 overflow-y-auto">
                {data.products.map((product) => (
                  <div key={product.id} className="flex items-center justify-between gap-3 rounded-md border border-atlas-line p-2 text-sm">
                    <div>
        ...
        <section className="rounded-md border border-atlas-line bg-white p-4 shadow-soft">
          <h2 className="mb-3 font-semibold">Fila operacional</h2>
          <div className="grid gap-3 md:grid-cols-2">
            {data.opsTasks.map((task) => (
              <div key={task.id} className="rounded-md border border-atlas-line p-3 text-sm">
                <div className="mb-2 flex items-center justify-between gap-3">
                  <span className="font-medium">{task.title}</span>
                  <span className="rounded-md bg-atlas-lavender px-2 py-1 font-mono text-[10px] uppercase tracking-[0.1em] text-atlas-violet">{task.status}</span>
                </div>
                <p className="text-atlas-body">{task.notes}</p>
                <p className="mt-2 text-xs text-atlas-muted">
                  {task.order.product.title} · {task.order.user.name ?? task.order.user.phone}
                </p>
              </div>
            ))}
            {!data.opsTasks.length && <p className="text-sm text-atlas-muted">Nenhuma tarefa operacional aberta.</p>}
          </div>
        </section>
        ...
        <section className="rounded-md border border-atlas-line bg-white p-4 shadow-soft">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {data.conversations.map((conversation) => (
              <div key={conversation.id} className="rounded-md border border-atlas-line p-3 text-sm">
                <div className="mb-2 flex items-center justify-between gap-3">
                  <span className="font-medium">{conversation.user.name ?? conversation.user.phone}</span>
                  <span className="rounded-md bg-atlas-lavender px-2 py-1 font-mono text-[10px] uppercase tracking-[0.1em] text-atlas-violet">{conversation.currentStep}</span>
                </div>
                <div className="space-y-1 text-atlas-body">
...
function Metric({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) {
  return (
    <div className="rounded-md border border-atlas-line bg-white p-4 shadow-soft">
      <div className="mb-3 text-atlas-violet">{icon}</div>
      <p className="text-2xl font-semibold">{value}</p>
      <p className="text-sm text-atlas-body">{label}</p>
    </div>
  );
}
